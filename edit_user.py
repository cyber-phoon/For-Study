...
class Edit_Windows():
    ...
    def edit_window(self, data):
        ...
        edit_win.title("Редактирование данных")
        bold_font = ("Tahoma", 10, "bold")
        edit_win.configure(bg="light blue")
        # создаем поля для ввода новых данных
        tk.Label(edit_win, text="Имя:", font=bold_font, bg="#87cefa", fg="black").grid(row=0, column=0)
        self.entry_new_first_name = tk.Entry(edit_win)
        self.entry_new_first_name.grid(row=0, column=1)
        tk.Label(edit_win, text="Фамилия:", font=bold_font, bg="#87cefa", fg="black").grid(row=1, column=0)
        self.entry_new_last_name = tk.Entry(edit_win)
        self.entry_new_last_name.grid(row=1, column=1)
        tk.Label(edit_win, text="Отчество:", font=bold_font, bg="#87cefa", fg="black").grid(row=2, column=0)
        self.entry_new_patronymic = tk.Entry(edit_win)
        self.entry_new_patronymic.grid(row=2, column=1)
        ...
        self.entry_new_first_name.insert(0, data[1])
        self.entry_new_last_name.insert(0, data[2])
        self.entry_new_patronymic.insert(0, data[3])
        #print(entry_new_first_name)
        #print(entry_new_last_name)
        #print(entry_new_patronymic)
        # создаем кнопку для того чтобы сделать фотографию и сохранить
        tk.Button(edit_win, text="Сделать фотографию", command=self.process_camera_photo, font=bold_font, bg="#87cefa", fg="black").grid()
        tk.Button(edit_win, text="Изменить введенное ФИО", command=lambda: self.change_data(self.entry_first_name.get(), self.entry_last_name.get(), self.entry_patronymic.get(), self.entry_new_first_name.get(), self.entry_new_last_name.get(), self.entry_new_patronymic.get()), font=bold_font, bg="#87cefa", fg="black").grid()
